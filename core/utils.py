...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]  ## 不同类别用不同的颜色
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick//2)[0]  ## 获取文字的高度和宽度
...
def nms(bboxes, iou_threshold, sigma=0.3, method='nms'):
    ...
    for cls in classes_in_img: ## 每个类别都进行nms
        ...
        while len(cls_bboxes) > 0:
            ...
            if method == 'soft-nms':
                weight = np.exp(-(1.0 * iou ** 2 / sigma))  ## iou大，降低置信度
...
def postprocess_boxes(pred_bbox, org_img_shape, input_size, score_threshold):
    ...
    pred_coor = np.concatenate([np.maximum(pred_coor[:, :2], [0, 0]),
                                np.minimum(pred_coor[:, 2:], [org_w - 1, org_h - 1])], axis=-1)  ## 限定框在图像内
    invalid_mask = np.logical_or((pred_coor[:, 0] > pred_coor[:, 2]), (pred_coor[:, 1] > pred_coor[:, 3]))  ## left坐标大于right坐标的框
    pred_coor[invalid_mask] = 0

    # # (4) discard some invalid boxes
    bboxes_scale = np.sqrt(np.multiply.reduce(pred_coor[:, 2:4] - pred_coor[:, 0:2], axis=-1)) ## 就是求面积的平方，代表box的尺度
    scale_mask = np.logical_and((valid_scale[0] < bboxes_scale), (bboxes_scale < valid_scale[1]))  ## 防止出现0和无穷
